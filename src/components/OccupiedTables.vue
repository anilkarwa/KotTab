<template>
<div id="app">
  <v-app id="inspire">
    <v-layout row>
      <v-flex xs12>
          <v-toolbar color="cyan" dark>
            <v-toolbar-title>{{ displayTitle($route.params.listTableType) }}</v-toolbar-title>
            <v-spacer></v-spacer>
          </v-toolbar>
            <v-container grid-list-md text-xs-center>
                <v-layout row wrap>
                    <v-flex v-for="table in items" :key="table.TBLName" @click="selectTable(table.TBLID)" xs3>
                        <img class="table-icon" src="data:image/svg+xml;utf8;base64," />
                        <p class="table-number">{{ table.TBLName }}</p>
                    </v-flex>
                </v-layout>
            </v-container>
      </v-flex>
    </v-layout>
  </v-app>
</div>
</template>
<script>
import axios from './Services/httpClient.js'
export default {
  data () {
    return {
      items: []
    }
  },
  beforeMount: function () {
    if (localStorage.getItem('TableType') === 'Occupied Tables') {
      console.log('YOOO-- Occupied Tables are now visible')
      this.getOccupiedTables()
    } else if (localStorage.getItem('TableType') === 'Vacant Tables') {
      console.log('YOOO-- Vacant Tables are now visible')
      this.getVacantTables()
    } else {
      console.log('Ye condition kabi nahe aayegi mere bhai')
    }
    // this.getOccupiedTables()
  },
  methods: {
    selectTable (tableNumber) {
      console.log('Selected Table Number', tableNumber)
    },
    displayTitle (name) {
      if (!(name === undefined)) {
        localStorage.setItem('TableType', name)
      }
      return localStorage.getItem('TableType')
    },
    getOccupiedTables () {
      axios.getOccupiedTables().then((data) => {
        console.log('Data Table Response', data)
        this.items = data
        console.log('Data in ITEM', this.items)
      })
    },
    getVacantTables () {
      axios.getVacantTables().then((data) => {
        console.log('Data Table in Response', data)
        this.items = data
        console.log('V: data in item', this.items)
      })
    }
  }
}
</script>
<style scoped>
     .table-cell {
         text-align: center;
     }
     .table-icon {
        border: 2px solid black;
        border-radius: 50%;
        padding: 7%;
     }
     .table-icon:hover {
        background-color: #00BDD2;
        border: 2px solid rgba(0, 0, 0, 0.25);

     }
     .table-number {
        font-weight: 400;
        font-size: 20px;
     }
</style>
